...
class ApiManager {
        ...
        public function getData($query)
        {
                ...
                foreach($xml->channel->item as $data){
                        ...
                        $result['pubDate'] = (string)$data->pubDate;
                        $book[] = $result;
                }
...
?>
      ...
      <div class=moviewrap id=<?echo $ida?> onclick="moveToWrite('<?echo $ida?>')">
		...
		&nbsp&nbsp&nbsp&nbsp <center><a class=movielink href="<?php echo $data['link'];?>" target="_blank"><?php echo $data['title'];?></a><br><?php
		echo "({$data['pubDate']})";
		?></center>

		<?php
		if ($data['description']){
		$show = str_replace("|", " ", $data['description']);
		echo "{$show}";}
		//if ($data['userRating']){
