...
moveonly = "<div id=moveonly><ul><li onclick='writer=false;movencopyThis(this);'>가져가기</li></ul></div>";
multimove = false;
timeset = false;
writer=true;
img=false;
...
$(function() {
toggleState('cardtoggles');toggleID('mycards');
toggleState('titletoggles');showID('newtitle');
pointsboxright = $(window).width()/2-450;
$('#pointsbox').css("right",pointsboxright);
...
$(window).bind('scroll',function () {
	...
	$( "#right" ).animate({	top: $(window).scrollTop()+50}, 0 );
	}else{$( "#right" ).css('top', 'inherit' );}
...
hideID("mainpage");
hideID("writesomething");
$("#first").html("<div class='wrapping'><div id='namewrap'>PUB와 친구그룹을 설정합니다. 추가한 후 체크하면 메뉴에 반영됩니다.</div></div>");
showID('first')
...
$(".popup").hide();
				...
				$.ajax({
						...
						showID("first");
						$("#first").html("<div class='wrapping'><div id='namewrap'>친구그룹을 구성합니다. 선택후 화살표를 눌러 이동합니다.</div></div>");
						}});
...
messageShow('PUB은 공개 게시판입니다.<br>모든 사용자가 글을 남기고 볼 수 있습니다.');
			...
			break;
					...
					$(".menu").css("padding-bottom","3px");
					$(".hiddenmenu").css("padding-bottom","3px");
					$("#menu"+$group).css("color","#"+groupcolorshow[$group]);
					$("#gline").css("background-color","#"+groupcolorshow[$group]);
					$("#menu"+$group).css("border-bottom","3px solid #"+groupcolorshow[$group]);
...
$("#friendsmove").appendTo("#save");
			...
			showID("loading");
						...
						$("#first").html("<div class='wrapping'><div id='namewrap'>"+html[1]+"님이 "  +html[2]+   "에 작성하신 글입니다.</div></div>");
						showID("first");
						hideID("writespin", "slide", 500);

						hideID("loading");
						$("#mainpage").html(html[0]);
						showID("mainpage");
						}
					}});
}
...
function moveLetter($onlynumber){
	$("#mod").hide();
	$("#moveonly").hide();

		if( $('#moveonly').length ==0){
		$("#mod"+$onlynumber).append(moveonly);
		}
		$("#moveonly").appendTo("#mod"+$onlynumber);
...
$('#mod').hide();
if ($($id).parent().parent().parent().parent().parent().find('textarea').length==0){

var titlevar = $($id).parent().parent().parent().parent().parent().find('.title').html();

var bodyvar = $($id).parent().parent().parent().parent().parent().find('.body').html();
bodyvar = bodyvar.replace(/<br>/g, "\r\n");
titlevar = titlevar.replace(/<br>/g, "\r\n");
$($id).parent().parent().parent().parent().parent().find('.title').replaceWith("<textarea class=writetitle placeholder='제목' onkeydow='javascript:if (event.keyCode == 13) return false;'>"+titlevar+"</textarea>");
$($id).parent().parent().parent().parent().parent().find('.body').replaceWith("<textarea  placeholder='내용' class=writebody>"+bodyvar+"</textarea>");
...
$($id).parent().parent().parent().parent().parent().find('.writetitle').focus();
$($id).parent().parent().parent().parent().parent().find('.writebody').focus();
event.stopPropagation();
$('textarea').autosize();
if (titlevar.length > 0){
$($id).parent().parent().parent().parent().parent().find('.writetitle').show();
}
...
$('#movemove').text('가져가기');
$('#movencopy > ul > li:eq('+(groupnumbernow+1)+')').hide();}
else{
$("#moveonly").hide();
$('#movemove').text('이동 / 복사 적용하기');
$('#movencopy > ul > li:eq('+(groupnumbernow+1)+')').show();
}
