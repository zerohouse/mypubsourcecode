...
$sql = mysql_query("SELECT * from t_$user_id where `id` = '$type[0]' and not `anony` = '1' order by no desc limit $i, 1");} //no  sort  id  name  talk  score  anony  talk_sort  only_no  date
	...
	$order='no';
	if ($eqs[0]==1){
	$select = $select. " AS A INNER JOIN score AS B ON A.only_no = B.only_no";
	if ($eqs[1]!=0){
	$eqorder[1] = "+ $eqs[1]*no";}
	if ($eqs[2]!=0){
	$eqorder[2] = "+ $eqs[2]*hum";}
	...
	$sql = mysql_query("SELECT *from $select where `sort` = '$group' order by $order desc limit $i, 1"); //no  sort  id  name  talk  score  anony  talk_sort  only_no  date
...
if ($data[0]!=null){
    ...
    echo "    <div class=replyonwrap>
        ...
        echo "</div></div>";}
			...
			else if ($type[1]==2){echo "<a id=mod$only_no[0] onclick='moveLetter($only_no[0])' class=mod></a>";}
