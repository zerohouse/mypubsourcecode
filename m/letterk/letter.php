...
<iframe class=myframe scrolling='no'  onload='javascript:resizeIframe(this);' src='mypage/mypage.html?id=$type[0]'>
	...
	$function="eqtype='$eq[0]';";
	$onoff = 'on';
	if ($eqs[0]==null||$eqs[0]==0){
	$onoff = 'off'; $style="style=opacity:0.3;";
...
$( '#pointseq' ).slider( 'option', 'disabled', true );";
	...
	$( '#sadeq' ).slider( 'option', 'value',". $eqs[6] .");";
		...
		<div class=wrapping>
            ...
            <div id='namewrap'>
               $user_data[2]님 안녕하세요. <font style=font-family:dek>Pub</font> {$groupname}입니다 &nbsp
           <a class=viewfriends onclick=toggleID('eql') style='cursor: hand;
float: right;
width: 30px;
height: 30px;
background-image: url(../icon/eq.png);
background-size: 30px;
top: 15;
position: absolute;'></a> </div> 
	<div id=eql class=letter style=display:none;>
	<div id=eqt onclick=eqOnoff();><a id=eqicon></a>Equalizer <a id=eqstate>$onoff</a></div>
	<div id=eqable $style>
	<div id=pointseq></div>
